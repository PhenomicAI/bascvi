...
def run_kni_on_folder(root_dir: str, cell_type_col: str = "standard_true_celltype"):
    ...
    cell_types = np.asarray(
        df_embeddings[cell_type_col].astype("category").cat.codes, dtype=int
    )
    ...
    results = pd.DataFrame(
        np.zeros((len(studies), run_names.shape[0])), index=studies, columns=run_names
    )
    ...
    for ii, fname in enumerate(tqdm(pred_paths)):
        ...
        df_embeddings = pd.read_csv(
            fname, delimiter="\t"
        )  # scVI / BAscVI / Harmony / Scanorama
        print(" - loaded embeddings")

        for i in range(dims[ii]):
            df_embeddings[cols[i]] = df_embeddings[cols[i]] - np.mean(
                df_embeddings[cols[i]]
            )
        ...
        cell_types = (
            ...
            - df_embeddings[cell_type_col].astype("category").cat.codes.min()
        )
        cat = df_embeddings["study_name"].astype("category")
        mapping = cat.cat.categories
        study_name = cat.cat.codes
        ...
        for study in studies:
            ...
            classifier.fit(
                df_embeddings[cols[: dims[ii]]].values[np.logical_not(study_mask), :],
                cell_types[np.logical_not(study_mask)],
            )

            pred = classifier.predict(
                df_embeddings[cols[: dims[ii]]].values[study_mask, :]
            )
