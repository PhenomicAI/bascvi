...
def get_stacked_protein_embeddings_matrix(protein_embeddings_dir, species_list, gene_list):
    ...
    genes_with_embeddings = {g: emb for s in protein_embeddings_dict.keys() for g, emb in protein_embeddings_dict[s] if g in gene_list}

    zero_embedding = np.zeros(genes_with_embeddings.values()[0].shape)
    protein_embeddings_matrix = [genes_with_embeddings[g].cpu().numpy() if g in genes_with_embeddings else zero_embedding for g in gene_list]
