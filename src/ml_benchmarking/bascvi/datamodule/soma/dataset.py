...
class TileDBSomaTorchIterDataset(IterableDataset):
    ...
    def __init__(
        ...
        """
        Args:
            soma_experiment_uri: URI to the SOMA experiment.
            obs_df: DataFrame with cell metadata (must have 'soma_joinid', 'cell_idx', etc.).
            num_input: Number of input genes/features.
            genes_to_use: Indices of genes to use.
            feature_presence_matrix: Matrix indicating feature presence per sample.
            block_size: Number of cells per block.
            num_workers: Number of data loader workers.
            num_modalities: Number of modalities (for training/validation).
            num_studies: Number of studies (for training/validation).
            num_samples: Number of samples (for training/validation).
            library_calcs: DataFrame with library size statistics.
            verbose: Print debug info if True.
            predict_mode: If True, disables training/validation-specific fields.
            pretrained_gene_indices: Indices for mapping to pretrained model input.
        """
        ...
        # Predict mode disables multi-batch fields
    ...
    def _calc_start_end(self, worker_id: int) -> tuple:
        """
        Calculate the start and end block indices for a given worker.
        Args:
            worker_id: Worker index.
        Returns:
            (start_block, end_block): Tuple of block indices.
        """
        # If fewer blocks than workers, assign one block per worker
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        """
        Initialize iteration, set worker-specific block range.
        Returns:
        ...
        # Reset counters at the start of each epoch (for persistent workers)
        self.block_counter = 0
        self.cell_counter = 0
        if self.verbose:
    ...
    def __next__(self) -> Dict[str, Any]:
        """
        Fetch the next data sample. Loads data block-wise and yields one cell at a time.
        Returns:
            Dictionary containing input features and metadata for one cell.
        Raises:
            StopIteration: When all blocks are exhausted.
        """
        if self.verbose:
        ...
        if self.curr_block < self.end_block:
            if self.cell_counter == 0:
                ...
                try:
                    # Read block from SOMA experiment
            ...
            if self.predict_mode:
                # Prediction mode: return minimal fields
                datum = {
                ...
                # Construct one-hot batch vectors
