...
class TileDBSomaTorchIterDataset(IterableDataset):
    ...
    def __init__(
        ...
        shuffle=False,
        prefetch_threads_per_worker=4  # New parameter to control prefetching threads
    ):     
    ...
    def __iter__(self):
        ...
        if self.verbose:
            print(f"Worker {self.worker_id} - start block: {self.start_block}, end block: {self.end_block}, block_size: {self.block_size}, prefetch threads: {len(self.prefetch_threads)}")
    ...
    def __next__(self):
        ...
        # Get next item from queue with timeout to prevent hanging
        try:
            datum = self.data_queue.get(timeout=300)  # 5-minute timeout
        except queue.Empty:
            print(f"Worker {self.worker_id} - Queue empty for too long, stopping iteration")
            raise StopIteration
        ...
        # Log more frequently when queue is getting empty
        queue_size = self.data_queue.qsize()
        if (queue_size < self.max_queue_size * 0.1 and time.time() - self.last_log_time > 5) or \
           (queue_size < self.max_queue_size * 0.01 and time.time() - self.last_log_time > 1):
            print(f"Worker {self.worker_id} - Queue size: {queue_size}, max size: {self.max_queue_size}")
