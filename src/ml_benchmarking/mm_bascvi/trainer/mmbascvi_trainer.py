...
class MMBAscVITrainer(pl.LightningModule):
    ...
    def __init__(
        ...
        # Create the model
        n_input = model_args.get("n_input")
        n_latent = model_args.get("n_latent")
        batch_level_sizes = model_args.get("batch_level_sizes")
        n_cell_type_experts = model_args.get("n_cell_type_experts")
        
        self.model = MMBAscVI(
            n_input=n_input,
            n_latent=n_latent,
            batch_level_sizes=batch_level_sizes,
            n_cell_type_experts=n_cell_type_experts
        )
    ...
    def compute_vae_kl_loss(self, mod_vae_params, cell_vae_params):
        ...
        kl_loss = mod_kl_loss + cell_kl_loss
    ...
    def compute_celltype_diversity_loss(self, z_celltype_list):
        ...
        # z_celltype_tensor shape: [num_cell_type_experts, batch_size, latent_dim]
        
        # Normalize embeddings along the latent dimension
        z_norm = F.normalize(z_celltype_tensor, p=2, dim=2)

        z_norm_transposed = z_norm.transpose(0, 1)
        # z_norm_transposed shape: [batch_size, num_cell_type_experts, latent_dim]
        ...
        similarity_matrix = torch.bmm(z_norm_transposed.transpose(1, 2), z_norm_transposed)
        ...
        # Return the mean of off-diagonal elements
        return (off_diagonal ** 2).sum() / (num_experts * (num_experts - 1))  # Minimize off-diagonal similarity
    ...
    def compute_loss(self, outputs, batch, optimizer_idx=0):
        ...
        if optimizer_idx == 0:
            ...
            # Reconstruction Loss
            reconstruction_loss = self.compute_reconstruction_loss(x_decoder_zinb_params, x, feature_presence_mask)
            ...
            for i in range(len(batch_logits_celltype_list)):    # iterate over celltype experts
                for j in range(len(batch_idx_dict.keys())):          # iterate over batch levels
                    batch_ct_class_loss_dict[batch_dict_keys[j]] += self.compute_batch_discriminator_loss(batch_logits_celltype_list[i][j], batch_idx_dict[batch_dict_keys[0]])
            ...
            for i in range(len(batch_idx_dict.keys())):
                batch_final_class_loss_dict[batch_dict_keys[i]] = self.compute_batch_discriminator_loss(batch_logits_final_list[i], batch_idx_dict[batch_dict_keys[i]])
